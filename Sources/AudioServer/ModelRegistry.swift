...
public let MODEL_REGISTRY: [ModelVariant] = [
    ...
    .init(name: "magnet-medium-30s-mlx-int8",
          ...
          kind: .music),
    // Only the published bundles are registered. The generic `sa3` aliases
    // used to point at the INT4 medium bundle, which was never uploaded, so
    // the most obvious name resolved to a repository that does not exist.
    .init(name: "stable-audio-3-dit-medium-mlx-int8",
          ...
          aliases: ["stable-audio-3", "sa3", "sa3-medium", "sa3-medium-int8"],
          kind: .music),
